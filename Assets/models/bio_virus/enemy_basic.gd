...
@onready var animation_player: AnimationPlayer = $mutant/AnimationPlayer
...
func TakeDamage (hurt_box : Hurt_box ) -> void:
	...
	if hp > 0:
		if hp % 5 == 0:
			enemy_damaged.emit(hurt_box)  # Also can trigger a stun here
		else:
			pass
	else:
...
#func spawn_blood(pos: Vector3) -> void:
	#var blood = preload("res://Levels/scenes/blood_spary.tscn").instantiate()
